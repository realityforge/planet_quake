...
//freeze
	...
	if ( attacker == cg.snap->ps.clientNum && !cg_noFragMessage.integer ) {
		char	*s;
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	int				clientNum;
	clientInfo_t	*ci;
	const char	worldsound[]="sound/world/";
...
//unlagged - attack prediction #2
	...
	case EV_GENERAL_SOUND:
		...
		if ( cgs.gameSounds[ es->eventParm ] ) {
			if (!cg_noMapSounds.integer || !strstr (CG_ConfigString( CS_SOUNDS + es->eventParm ), worldsound)) trap_S_StartSound (NULL, es->number, CHAN_VOICE, cgs.gameSounds[ es->eventParm ] );
		} else {
			s = CG_ConfigString( CS_SOUNDS + es->eventParm );
			if (!cg_noMapSounds.integer || !strstr (s, worldsound)) trap_S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, s ) );
		}
		break;

	case EV_GLOBAL_SOUND:	// play from the player's head so it never diminishes
		DEBUGNAME("EV_GLOBAL_SOUND");
		if (!cg_noMapSounds.integer) { if ( cgs.gameSounds[ es->eventParm ] ) {

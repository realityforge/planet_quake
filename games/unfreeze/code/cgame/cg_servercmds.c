...
void CG_ParseServerinfo( void ) {
	...
	if (cg_legit.integer && strcmp (Info_ValueForKey(info, "gamename"), "unfreeze") && strcmp (Info_ValueForKey(info, "unfreeze"), "comp")) {
		Com_Error (ERR_DISCONNECT, "that is not a UnFreeZe server!");
		return;
	}
	if (!strcmp (Info_ValueForKey(info, "gamename"), "baseq3") || !strcmp (Info_ValueForKey(info, "gamename"), "osp") || (*Info_ValueForKey(info, "g_baseq3") && strcmp (Info_ValueForKey(info, "g_baseq3"), "0"))) {
		baseq3 = qtrue;
		if (!strcmp (Info_ValueForKey(info, "gamename"), "osp") || (*Info_ValueForKey(info, "g_osp") && strcmp (Info_ValueForKey(info, "g_osp"), "0"))) osp = qtrue; else osp = qfalse;
	} else { baseq3 = qfalse; osp = qfalse; }
...
void CG_SetConfigValues( void ) {
	...
	s=CG_ConfigString( CS_SCORES1P );
	if (*s) cgs.scores1p = atoi( s ); else cgs.scores1p=SCORE_NOT_PRESENT;
	s=CG_ConfigString( CS_SCORES2P );
	if (*s) cgs.scores2p = atoi( s ); else cgs.scores2p=SCORE_NOT_PRESENT;
...
static void CG_ConfigStringModified( void ) {
	...
	} else if ( num == CS_SCORES1P ) {
		cgs.scores1p = atoi( str );
	} else if ( num == CS_SCORES2P ) {
		cgs.scores2p = atoi( str );
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "chat" ) ) {
		if ( !cg_teamChatsOnly.integer ) {
			if ( cg_nochatbeep.integer < 1 ) trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	...
	if ( !strcmp( cmd, "tchat" ) ) {
		if ( cg_nochatbeep.integer < 2 ) trap_S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
		Q_strncpyz( text, CG_Argv(1), MAX_SAY_TEXT );
		CG_RemoveChatEscapeChar( text );
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
	...
	if (strcmp(cmd,"statsinfo") && strcmp(cmd,"xstats1")) CG_Printf( "Unknown client game command: %s\n", cmd );
}

...
static qboolean	CG_FindClientModelFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		switch ( ci->team ) {
			case TEAM_BLUE: {
				if (cgs.clientinfo[cg.clientNum].team == TEAM_RED && cg_forceSkin.integer) team = "default"; else team = "blue";
				break;
			}
			case TEAM_RED: {
				if (cgs.clientinfo[cg.clientNum].team == TEAM_BLUE && cg_forceSkin.integer) team = "default"; else team = "red";
				break;
			}
			default: {
				team = "default";
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	Com_sprintf( filename, sizeof( filename ), "models/players/%s/animation.cfg", headName ); CG_ParseHeadAnimationFile ( filename, ci );
...
static void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	if ((ci-cgs.clientinfo) == cg.clientNum)
		for ( i = 0, ci2 = cgs.clientinfo ; i < cgs.maxclients ; i++, ci2++ )
			if ( ci2 != ci && ci2->infoValid ) CG_LoadClientInfo (ci2);
	ci->deferred = qfalse;
...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( cg_forceModel.integer && newInfo.team != TEAM_SPECTATOR && clientNum != cg.clientNum ) {
...
//freeze
			if ( cg_forceModel.integer & 2 ) {
				if (cgs.clientinfo[cg.clientNum].team == newInfo.team) {
					if ( cg_forceModel.integer & 1) trap_Cvar_VariableStringBuffer( "team_model", modelStr, sizeof ( modelStr ) );
					else Q_strncpyz( modelStr, v, sizeof( modelStr ) );
				}
				else trap_Cvar_VariableStringBuffer( "enemy_model", modelStr, sizeof ( modelStr ) );
			} else trap_Cvar_VariableStringBuffer( "team_model", modelStr, sizeof ( modelStr ) );
...
//freeze
			...
			trap_Cvar_VariableStringBuffer( (cg_forceModel.integer!=1 && cg_forceModel.integer!=-1) ? "enemy_model" : "model", modelStr, sizeof( modelStr ) );
...
//freeze
			if ( cg_forceModel.integer & 2 ) {
				if (cgs.clientinfo[cg.clientNum].team == newInfo.team) {
					if ( cg_forceModel.integer & 1) trap_Cvar_VariableStringBuffer( "team_headmodel", modelStr, sizeof ( modelStr ) );
					else Q_strncpyz( modelStr, v, sizeof( modelStr ) );
				}
				else trap_Cvar_VariableStringBuffer( "enemy_headmodel", modelStr, sizeof ( modelStr ) );
			} else trap_Cvar_VariableStringBuffer( "team_headmodel", modelStr, sizeof ( modelStr ) );
			slash = strchr( modelStr, '/' );
			if ( slash ) {
				*slash = 0;
			}
...
//freeze
			...
			trap_Cvar_VariableStringBuffer( (cg_forceModel.integer!=1 && cg_forceModel.integer!=-1) ? "enemy_headmodel" : "headmodel", modelStr, sizeof( modelStr ) );
			if ( ( skin = strchr( modelStr, '/' ) ) == NULL) {
				skin = "default";
			} else {
				*skin++ = 0;
	...
	*ci = newInfo;
	...
	if (clientNum == cg.clientNum) {
		...
		for (i=0; i<cgs.maxclients; i++) if (i!=clientNum && cgs.clientinfo[i].infoValid) CG_NewClientInfo (i);
...
static void CG_PlayerFloatSprite( centity_t *cent, qhandle_t shader ) {
	...
	ent.radius = 10;
	ent.renderfx = rf;
	if (cg_drawFriend.integer > 1 && shader == cgs.media.friendShader) {
		if (cg_friendMarkColor.integer) {
			ent.shaderRGBA[0] = 255*g_color_table[cg_friendMarkColor.integer%8][0];
			ent.shaderRGBA[1] = 255*g_color_table[cg_friendMarkColor.integer%8][1];
			ent.shaderRGBA[2] = 255*g_color_table[cg_friendMarkColor.integer%8][2];
		} else {
...
if (cg_drawRewards.integer) {
	...
	if ( !(cent->currentState.eFlags & EF_DEAD) && 
		...
		if (cg_drawFriend.integer) { if (!cg_frozenFriendMark.integer || !Q_Isfreeze(cent->currentState.clientNum))
			CG_PlayerFloatSprite( cent, cgs.media.friendShader );
		else CG_PlayerFloatSprite( cent, cgs.media.frozenFriendShader ); }
		return;
	}
}

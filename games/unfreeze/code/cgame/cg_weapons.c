...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_LIGHTNING:
		MAKERGB( weaponInfo->flashDlightColor, 0.6f, 0.6f, 1.0f );
		if (!cg_noWeaponSounds.integer) weaponInfo->readySound = trap_S_RegisterSound( "sound/weapons/melee/fsthum.wav", qfalse );
		weaponInfo->firingSound = trap_S_RegisterSound( "sound/weapons/lightning/lg_hum.wav", qfalse );

		weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/lightning/lg_fire.wav", qfalse );
		if (cg_lgrcore.integer) cgs.media.lightningShader = trap_R_RegisterShader( "lgrcorenpm"); else cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNewNPM");
	...
	case WP_GRAPPLING_HOOK:
		...
		weaponInfo->firingSound = trap_S_RegisterSound( "sound/weapons/melee/fstrun.wav", qfalse );
//freeze
		cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNewNPM" );
//freeze
		break;
...
//		weaponInfo->missileModel = cgs.media.invulnerabilityPowerupModel;
	...
	case WP_RAILGUN:
		if (!cg_noWeaponSounds.integer) weaponInfo->readySound = trap_S_RegisterSound( "sound/weapons/railgun/rg_hum.wav", qfalse );
		MAKERGB( weaponInfo->flashDlightColor, 1, 0.5f, 0 );
		if (cg_oldRailSound.integer) weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/railgun/railgf1a.wav", qfalse ); else weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/railgun/railgf1anew.wav", qfalse );
	...
	case WP_BFG:
		if (!cg_noWeaponSounds.integer) weaponInfo->readySound = trap_S_RegisterSound( "sound/weapons/bfg/bfg_hum.wav", qfalse );
...
//unlagged - true lightning
	...
	if ( trace.fraction < 1.0 && !cg_noExplosions.integer ) {
		vec3_t	angles;
		vec3_t	dir;
...
//	int	col;
	...
	if (!flash.hModel || (ps && nonPredictedCent->currentState.weapon != WP_LIGHTNING)) {
		return;
	}
	angles[YAW] = 0;
	angles[PITCH] = 0;
	...
	if (!ps) trap_R_AddRefEntityToScene( &flash );
...
void CG_DrawWeaponSelect( void ) {
	...
	if ( cg.time > cg.weaponSelectTime + cg_weaponSelectTime.value*1000 ) {
		return;
	}
	trap_R_SetColor( g_color_table[7] );
	...
	for ( i = 1 ; i < 16 ; i++ ) {
		...
		} else if (cg.snap->ps.ammo[i]>0) {
			name=va("%i", cg.snap->ps.ammo[i]);
			CG_DrawSmallStringColor (x+16-strlen(name)*4, y+8, name, g_color_table[0]);
		}
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType ) {
	...
	//
	if ( mod && !cg_noExplosions.integer ) {

...
void CG_AddMarks( void ) {
	...
	for ( ; mp != &cg_activeMarkPolys ; mp = next ) {
		...
		if ( cg.time > mp->time + cg_marksTime.integer ) {
			CG_FreeMarkPoly( mp );
			continue;
		}
		...
		if ( mp->markShader == cgs.media.energyMarkShader ) {
			...
			fade = 450 - 450 * ( (cg.time - mp->time ) / 2700.0 );
			if ( fade < 255 ) {
				if ( fade < 0 ) {
					fade = 0;
				}
		...
		t = mp->time + cg_marksTime.integer - cg.time;
		if ( t < MARK_FADE_TIME ) {

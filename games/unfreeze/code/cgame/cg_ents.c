...
static void CG_EntityEffects( centity_t *cent ) {
	...
	// add loop sound
	if ( cent->currentState.loopSound && !cg_noMapSounds.integer ) {
...
//unlagged - timenudge extrapolation
	...
	if ( cent->currentState.number < MAX_CLIENTS &&
		...
		cent->currentState.pos.trDuration = 30; // 1000 / sv_fps.integer;
	}
//unlagged - timenudge extrapolation
...
//unlagged - projectile nudge
	...
	if ( cent->currentState.eType == ET_MISSILE && cent->currentState.weapon != WP_GRAPPLING_HOOK ) {
		...
		if ( cent->currentState.otherEntityNum == cg.clientNum ) {
			...
			timeshift = 30; // 1000 / sv_fps.integer;
		}
		// if it's not, and it's not a grenade launcher
			...
			timeshift = cg_projectileNudge.integer + 30; // 1000 / sv_fps.integer;
		}
	}
...
static void CG_AddCEntity( centity_t *cent ) {
	...
	case ET_PORTAL:
		...
		break;
	case ET_SPEAKER:
		if (!cg_noMapSounds.integer) CG_Speaker( cent );
		break;
	case ET_GRAPPLE:
		CG_Grapple( cent );
		break;

...
static char quake3worldMessage[] = "Visit forum.unfreeze.xyz - News, Community, Events, Files";
...
static void ArenaServers_UpdateMenu( void ) {
	...
	for( i = 0, j = 0; i < count; i++, servernodeptr++ ) {
		...
		Com_sprintf( buff, MAX_LISTBOXWIDTH, "%-20.20s %-12.12s %2d/%2d %-8.8s %3s %s%3d " S_COLOR_YELLOW "%s", 
			...
			netnames[servernodeptr->nettype], pingColor, servernodeptr->pingtime, servernodeptr->location );
		j++;
	}
...
static void ArenaServers_Insert( char* adrstr, char* info, int pingtime )
{
	...
	if (((pingtime >= ArenaServers_MaxPing()) && (g_servertype != AS_FAVORITES)) || (strcmp (Info_ValueForKey( info, "game" ), "unfreeze") && strcmp (Info_ValueForKey( info, "uf" ), "c")))
	{
	...
	Q_strncpyz( servernodeptr->location, Info_ValueForKey( info, "l"), 3 );
	...
	*/
	...
	if( *s ) {
		...
		if (strcmp(s,"unfreeze")) Q_strncpyz( servernodeptr->gamename, "ufcomp", sizeof(servernodeptr->gamename) ); else Q_strncpyz( servernodeptr->gamename, "unfreeze", sizeof(servernodeptr->gamename) );
	}
	else {
...
static void ArenaServers_DoRefresh( void )
{
	...
	// trigger at 200Hz intervals
	g_arenaservers.nextpingtime = uis.realtime + 5;
...
static void ArenaServers_MenuInit( void ) {
	...
	g_arenaservers.punkbuster.generic.flags			= QMF_PULSEIFFOCUS|QMF_SMALLFONT|QMF_GRAYED;

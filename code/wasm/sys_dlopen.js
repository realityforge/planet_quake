...
function getHandler (obj, symName) {
  if (!GOT[symName]) {
...
function loadWebAssemblyModule (binary, flags) {
  ...
  function loadModule() {
    ...
    */
    ...
    function getProxyHandler(obj, prop) {
    ...
    var proxy = new Proxy(env, getProxyHandler);
    var info = {
      'GOT.mem': new Proxy(asmLibraryArg, getHandler),
      'GOT.func': new Proxy(asmLibraryArg, getHandler),
      'env': proxy,
    };
...
function dlopen (filenameAddr, flags) {
...
function dladdr (addr, info) {
  // report all function pointers as coming from this program itself XXX not really correct in any way
  var fname = stringToNewUTF8(getExecutableName()); // XXX leak
  HEAP32[(info+0)>>2] = fname;
  HEAP32[(info+Runtime.QUANTUM_SIZE)>>2] = 0;
  HEAP32[(info+Runtime.QUANTUM_SIZE*2)>>2] = 0;
  HEAP32[(info+Runtime.QUANTUM_SIZE*3)>>2] = 0;
  return 1;
}

function Sys_LoadLibrary (name) {
	if ( _FS_AllowedExtension( name, false, void 0 ) )
	{
		Sys_Error( ERR_FATAL, "Sys_LoadLibrary: Unable to load library with '%s' extension", ext )
	}

  var handle = _dlopen(name, 2)
  return handle
}

...
function isSecured (socksServer) {
...
function Sys_Main_PlatformInit () {
  ...
  Object.assign(Module, {
    websocket: Object.assign(Module.websocket || {}, {
      url: SYSM.isSecured(Cvar_VariableString('net_socksServer'))
      ? 'wss://'
      : 'ws://'
    })
  })
...
function Sys_Main_PlatformExit () {
  SYSC.returnURL = Cvar_VariableString('cl_returnURL')
...
function Sys_Main_Main_Milliseconds () {
  if (!SYSM.timeBase) {
...
function Sys_Main_GetCurrentUser () {
  return allocate(intArrayFromString('player'), ALLOC_STACK)
}

function Sys_Main_Dialog (type, message, title) {

}

function Sys_Main_ErrorDialog (error) {
...
function Sys_Main_CmdArgs () {
  var argv = ['quake3e'].concat(SYSM.getQueryCommands())
...
function Sys_Main_CmdArgsC () {
  return SYSM.getQueryCommands().length + 1
}

function Sys_Main_DownloadLocalFile (fileName) {
...
function Sys_Main_LoadLibrary (filenameAddr) {
  if ( _FS_AllowedExtension( filenameAddr, false, void 0 ) )
  {
    Sys_Error( 'fatal', 'Sys_Main_LoadLibrary: Unable to load library with ' + ext + ' extension' )
  }
  ...
  try {
    ...
    Sys_Error( 'fatal', 'Sys_Main_LoadLibrary: Could not load dynamic lib: ' + filename + '\n' + e );
    ProxyCallback(0)
  }
  return 0
}

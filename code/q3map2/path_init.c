...
void InitPaths( int *argc, char **argv ){
	...
	for ( i = 0; i < *argc; i++ )
	{
		...
		if ( strcmp( argv[ i ], "-game" ) == 0 ) {
			if ( ++i >= *argc ) {
				Com_Error(ERR_DROP, "Out of arguments: No game specified after %s", argv[ i - 1 ] );
			}
			argv[ i - 1 ] = NULL;
			game = GetGame( argv[ i ] );
			if ( game == NULL ) {
		...
		/* -fs_basepath */
			...
			if ( ++i >= *argc ) {
				Com_Error(ERR_DROP, "Out of arguments: No path specified after %s.", argv[ i - 1 ] );
			}
			argv[ i - 1 ] = NULL;
			AddBasePath( argv[ i ] );
			argv[ i ] = NULL;
	...
	*argc = k;
	...
	if ( numBasePaths == 0 ) {
		...
		/* check again */
		if ( numBasePaths == 0 ) {
			Com_Error(ERR_DROP, "Failed to find a valid base path." );
		}
	}

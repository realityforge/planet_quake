...
//				ptri->colors[j][k] = tri.pt[j].c.v[k];
		...
		if ( ( ptri - tripool ) >= POLYSET_MAXTRIANGLES ) {
			Com_Error(ERR_DROP, "Error: too many triangles; increase POLYSET_MAXTRIANGLES\n" );
		}
	}
}
...
void TRI_LoadPolysets( const char *filename, polyset_t **ppPSET, int *numpsets ){
	...
	*( (unsigned char *)&exitpattern + 3 ) = *( (unsigned char *)&t + 0 );
	...
	if ( ( input = fopen( filename, "rb" ) ) == 0 ) {
		Com_Error(ERR_DROP, "reader: could not open file '%s'", filename );
	}
	...
	if ( BigLong( magic ) != MAGIC ) {
		Com_Error(ERR_DROP, "%s is not a Alias object separated triangle file, magic number is wrong.", filename );
	}
...
//
		if ( count > 0 ) {
			...
			if ( ++pset >= POLYSET_MAXPOLYSETS ) {
				Com_Error(ERR_DROP, "Error: too many polysets; increase POLYSET_MAXPOLYSETS\n" );
			}
		}
	}

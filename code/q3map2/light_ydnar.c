...
static void SubsampleRawLuxel_r( rawLightmap_t *lm, mapTrace_t *trace, vec3_t sampleOrigin, int x, int y, float bias, float *lightLuxel ){
...
void FloodLightRawLightmap( int rawLightmapNum ){
	...
	memset( &trace,0,sizeof( mapTrace_t ) );
	/* setup trace */
	trace.testOcclusion = qtrue;
	trace.forceSunlight = qfalse;
	trace.twoSided = qtrue;

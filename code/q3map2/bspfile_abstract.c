...
void IncDrawVerts(){
	...
	if ( bspDrawVerts == 0 ) {
		...
		if ( !newBspDrawVerts ) {
			free (bspDrawVerts);
			Com_Error(ERR_DROP, "realloc() failed (IncDrawVerts)" );
		}
...
int GetLumpElements( bspHeader_t *header, int lump, int size ){
	...
	if ( header->lumps[ lump ].length % size ) {
		if ( force ) {
			Com_Printf( S_COLOR_YELLOW "WARNING: GetLumpElements: odd lump size (%d) in lump %d\n", header->lumps[ lump ].length, lump );
			return 0;
		}
		else{
			Com_Error(ERR_DROP, "GetLumpElements: odd lump size (%d) in lump %d", header->lumps[ lump ].length, lump );
		}
	}
...
int CopyLump( bspHeader_t *header, int lump, void *dest, int size ){
	...
	if ( length % size ) {
		if ( force ) {
			Com_Printf( S_COLOR_YELLOW "WARNING: CopyLump: odd lump size (%d) in lump %d\n", length, lump );
			return 0;
		}
		else{
			Com_Error(ERR_DROP, "CopyLump: odd lump size (%d) in lump %d", length, lump );
		}
	}
...
void LoadBSPFile( const char *filename ){
	...
	if ( game == NULL || game->load == NULL ) {
		Com_Error(ERR_DROP, "LoadBSPFile: unsupported BSP file format" );
	}
...
void WriteBSPFile( const char *filename ){
	...
	if ( game == NULL || game->write == NULL ) {
		Com_Error(ERR_DROP, "WriteBSPFile: unsupported BSP file format" );
	}
...
qboolean ParseEntity( void ){
	...
	if ( numEntities == MAX_MAP_ENTITIES ) {
		Com_Error(ERR_DROP, "numEntities == MAX_MAP_ENTITIES" );
	}

...
fixedWinding_t *NewFixedWinding( int points ){
	...
	if ( points > MAX_POINTS_ON_WINDING ) {
		Com_Error(ERR_DROP, "NewWinding: %i points", points );
	}
...
void ClusterMerge( int leafnum ){
	...
	for ( i = 0; i < leaf->numportals; i++ )
	{
		...
		if ( p->status != stat_done ) {
			Com_Error(ERR_DROP, "portal not done" );
		}
...
void LoadPortals( char *name ){
	...
	else
	{
		f = fopen( name, "r" );
		if ( !f ) {
			Com_Error(ERR_DROP, "LoadPortals: couldn't read %s\n",name );
		}
	}

	if ( fscanf( f,"%79s\n%i\n%i\n%i\n",magic, &portalclusters, &numportals, &numfaces ) != 4 ) {
		Com_Error(ERR_DROP, "LoadPortals: failed to read header" );
	}
	...
	if ( numBSPVisBytes > MAX_MAP_VISIBILITY ) {
		Com_Error(ERR_DROP, "MAX_MAP_VISIBILITY exceeded" );
	}
	...
	for ( i = 0, p = portals ; i < numportals ; i++ )
	{
		if ( fscanf( f, "%i %i %i ", &numpoints, &leafnums[0], &leafnums[1] ) != 3 ) {
			Com_Error(ERR_DROP, "LoadPortals: reading portal %i", i );
		}
		if ( numpoints > MAX_POINTS_ON_WINDING ) {
			Com_Error(ERR_DROP, "LoadPortals: portal %i has too many points", i );
		}
		...
		if ( fscanf( f, "%i ", &hint ) != 1 ) {
			Com_Error(ERR_DROP, "LoadPortals: reading hint state" );
		}
		...
		if ( l->numportals == MAX_PORTALS_ON_LEAF ) {
			Com_Error(ERR_DROP, "Leaf with too many portals" );
		}
	...
	for ( i = 0, p = faces; i < numfaces; i++ )
	{
		...
		l->merged = -1;
		if ( l->numportals == MAX_PORTALS_ON_LEAF ) {
			Com_Error(ERR_DROP, "Leaf with too many faces" );
		}
		l->portals[l->numportals] = p;
		l->numportals++;
...
int VisMain( int argc, char **argv ){
	...
	if ( i != argc - 1 ) {
		Com_Error(ERR_DROP, "usage: vis [-threads #] [-level 0-4] [-fast] [-v] bspfile" );
	}

...
image_t *ImageLoad( const char *filename ){
	...
	/* too many images? */
	if ( image == NULL ) {
		Com_Error(ERR_DROP, "MAX_IMAGES (%d) exceeded, there are too many image files referenced by the map.", MAX_IMAGES );
	}
	...
	if ( size <= 0 || image->width <= 0 || image->height <= 0 || image->pixels == NULL ) {
		//%	Com_Printf( "size = %d  width = %d  height = %d  pixels = 0x%08x (%s)\n",

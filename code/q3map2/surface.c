...
mapDrawSurface_t *AllocDrawSurface( surfaceType_t type ){
	...
	if ( type <= SURFACE_BAD || type >= NUM_SURFACE_TYPES ) {
		Com_Error(ERR_DROP, "AllocDrawSurface: Invalid surface type %d specified", type );
	}

	/* bounds check */
	if ( numMapDrawSurfs >= MAX_MAP_DRAW_SURFS ) {
		Com_Error(ERR_DROP, "MAX_MAP_DRAW_SURFS (%d) exceeded", MAX_MAP_DRAW_SURFS );
	}
...
mapDrawSurface_t *DrawSurfaceForSide( bspEntity_t *e, brush_t *b, side_t *s, winding_t *w ){
	...
	if ( w->numpoints > MAX_POINTS_ON_WINDING ) {
		Com_Error(ERR_DROP, "DrawSurfaceForSide: w->numpoints = %d (> %d)", w->numpoints, MAX_POINTS_ON_WINDING );
	}
...
static void SubdivideFace_r( bspEntity_t *e, brush_t *brush, side_t *side, winding_t *w, int fogNum, float subdivisions ){
	...
	if ( w->numpoints < 3 ) {
		Com_Error(ERR_DROP, "SubdivideFace_r: Bad w->numpoints (%d < 3)", w->numpoints );
	}
...
static int FilterTrianglesIntoTree( mapDrawSurface_t *ds, tree_t *tree ){
	...
	for ( i = 0; i < ds->numIndexes; i += 3 )
	{
		...
		if ( ds->indexes[ i ] >= ds->numVerts ||
			...
			Com_Error(ERR_DROP, "Index %d greater than vertex count %d", ds->indexes[ i ], ds->numVerts );
		}
...
void EmitDrawVerts( mapDrawSurface_t *ds, bspDrawSurface_t *out ){
	...
	for ( i = 0; i < ds->numVerts; i++ )
	{
		...
		if ( numBSPDrawVerts == MAX_MAP_DRAW_VERTS ) {
			Com_Error(ERR_DROP, "MAX_MAP_DRAW_VERTS" );
		}
		IncDrawVerts();
	...
	if ( out->firstIndex == numBSPDrawIndexes ) {
		...
		for ( i = 0; i < ds->numIndexes; i++ )
		{
			if ( numBSPDrawIndexes == MAX_MAP_DRAW_INDEXES ) {
				Com_Error(ERR_DROP, "MAX_MAP_DRAW_INDEXES" );
			}
...
static void MakeDebugPortalSurfs_r( node_t *node, shaderInfo_t *si ){
	...
	for ( c = 0, p = node->portals; p != NULL; c++, p = p->next[ s ] )
	{
		...
		if ( w && p->nodes[ 0 ] == node ) {
			...
			/* check max points */
			if ( w->numpoints > 64 ) {
				Com_Error(ERR_DROP, "MakePortalSurfs_r: w->numpoints = %d", w->numpoints );
			}

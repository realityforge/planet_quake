	...
	while ( 1 )
	{
		...
		else
		{
			...
			if ( surfaceNum < 0 || surfaceNum > MAX_MAP_DRAW_SURFS ) {
				Com_Error(ERR_DROP, "ReadSurfaceExtraFile(): %s, line %d: bogus surface num %d", srfPath, scriptline, surfaceNum );
			}
		...
		if ( !GetToken( qtrue ) || strcmp( token, "{" ) ) {
			Com_Error(ERR_DROP, "ReadSurfaceExtraFile(): %s, line %d: { not found", srfPath, scriptline );
		}
		while ( 1 )
		{
			if ( !GetToken( qtrue ) ) {

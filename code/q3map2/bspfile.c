...
int GetLumpElements( bspHeader_t  *header, int lump, int size ) {
	...
	if ( length % size ) {
		Com_Error(ERR_DROP, "LoadBSPFile: odd lump size" );
	}
...
void    LoadBSPFile( const char *filename ) {
	...
	if ( header->ident != BSP_IDENT ) {
		Com_Error(ERR_DROP, "%s is not a IBSP file", filename );
	}
	if ( header->version != bsp_version ) {
		Com_Error(ERR_DROP, "%s is version %i, not %i", filename, header->version, bsp_version );
	}

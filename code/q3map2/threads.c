...
void ThreadLock( void ){
	...
	if ( enter ) {
		Com_Error(ERR_DROP, "Recursive ThreadLock\n" );
	}
	enter = 1;
}
...
void ThreadUnlock( void ){
	...
	if ( !enter ) {
		Com_Error(ERR_DROP, "ThreadUnlock without lock\n" );
	}
	enter = 0;
...
void RunThreadsOn( int workcnt, qboolean showpacifier, void ( *func )( int ) ){
	...
	if ( !my_mutex ) {
		...
		if ( pthread_mutexattr_create( &mattrib ) == -1 ) {
			Com_Error(ERR_DROP, "pthread_mutex_attr_create failed" );
		}
		if ( pthread_mutexattr_setkind_np( &mattrib, MUTEX_FAST_NP ) == -1 ) {
			Com_Error(ERR_DROP, "pthread_mutexattr_setkind_np failed" );
		}
		if ( pthread_mutex_init( my_mutex, mattrib ) == -1 ) {
			Com_Error(ERR_DROP, "pthread_mutex_init failed" );
		}
	}

	if ( pthread_attr_create( &attrib ) == -1 ) {
		Com_Error(ERR_DROP, "pthread_attr_create failed" );
	}
	if ( pthread_attr_setstacksize( &attrib, 0x100000 ) == -1 ) {
		Com_Error(ERR_DROP, "pthread_attr_setstacksize failed" );
	}
	...
	for ( i = 0 ; i < numthreads ; i++ )
	{
		if ( pthread_create( &work_threads[i], attrib
			...
			Com_Error(ERR_DROP, "pthread_create failed" );
		}
	}
	...
	for ( i = 0 ; i < numthreads ; i++ )
	{
		if ( pthread_join( work_threads[i], &status ) == -1 ) {
			Com_Error(ERR_DROP, "pthread_join failed" );
		}
	}
...
void RunThreadsOn( int workcnt, qboolean showpacifier, void ( *func )( int ) ){
	...
	for ( i = 0 ; i < numthreads - 1 ; i++ )
	{
		...
		if ( pid[i] == -1 ) {
			perror( "sproc" );
			Com_Error(ERR_DROP, "sproc failed" );
		}
	}
...
void recursive_mutex_init( pthread_mutexattr_t attribs ){
	...
	if ( pthread_cond_init( &pt_mutex->cond, NULL ) != 0 ) {
		Com_Error(ERR_DROP, "pthread_cond_init failed\n" );
	}
...
void RunThreadsOn( int workcnt, qboolean showpacifier, void ( *func )( int ) ){
	...
	else
	{
		...
		if ( pthread_mutexattr_init( &mattrib ) != 0 ) {
			Com_Error(ERR_DROP, "pthread_mutexattr_init failed" );
		}
...
#endif
		{ Com_Error(ERR_DROP, "pthread_mutexattr_settype failed" ); }

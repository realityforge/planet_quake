...
int WSAGetLastCom_Error(ERR_DROP,){
	return errno;
}
 ...
 */
	...
	/*
	    ...
	    Sys_Com_Error(ERR_DROP,"WINS_Init: you must specify a number after -udpport");
	 */
	...
	if ( local == 0 ) {
		WinCom_Error(ERR_DROP, "WINS_Init: Unable to resolve hostname\n" );
		return 0;
	}
...
//		Cvar_Set ("hostname", buff);
	...
	if ( ( net_controlsocket = WINS_OpenSocket( 0 ) ) == SOCKET_ERROR ) {
		WinCom_Error(ERR_DROP, "WINS_Init: Unable to open control socket\n" );
	}
   ...
   void WINS_Listen(int state)
   {
    ...
    if (state)
    {
        ...
        if ((net_acceptsocket = WINS_OpenSocket (net_hostport)) == -1)
            WinCom_Error(ERR_DROP,"WINS_Listen: Unable to open accept socket\n");
        return;
    }
...
int WINS_OpenSocket( int port ){
	...
	if ( ( newsocket = socket( PF_INET, SOCK_DGRAM, IPPROTO_UDP ) ) == SOCKET_ERROR ) {
		WinPrint( "WINS_OpenSocket: %s\n", WINS_ErrorMessage( WSAGetLastCom_Error(ERR_DROP,) ) );
		return -1;
	} //end if
...
int WINS_Read( int socket, byte *buf, int len, struct sockaddr_s *addr ){
	...
	if ( addr ) {
		...
		if ( ret == -1 ) {
//			errno = WSAGetLastCom_Error(ERR_DROP,);
...
int WINS_Broadcast( int socket, byte *buf, int len ){
	...
	if ( socket != net_broadcastsocket ) {
		if ( net_broadcastsocket != 0 ) {
			WinCom_Error(ERR_DROP, "Attempted to use multiple broadcasts sockets\n" );
		}
...
int WINS_Write( int socket, byte *buf, int len, struct sockaddr_s *addr ){
	...
	if ( addr ) {
		...
		while ( written < len )
		{
			...
			if ( ret == SOCKET_ERROR ) {
				if ( WSAGetLastCom_Error(ERR_DROP,) != EAGAIN ) {
					return qfalse;
				}

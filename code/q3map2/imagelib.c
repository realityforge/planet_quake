...
int bufLittleShort( byte *buf, int len, int *pos ){
	...
	if ( ( len - *pos ) < 2 ) {
		Com_Error(ERR_DROP, "Unexpected buffer end" );
	}
...
byte  *LBMRLEDecompress( byte *source,byte *unpacked, int bpwidth ){
	...
	if ( count > bpwidth ) {
		Com_Error(ERR_DROP, "Decompression exceeded width!\n" );
	}
...
//
	...
	if ( *(int *)LBMbuffer != LittleLong( FORMID ) ) {
		Com_Error(ERR_DROP, "No FORM ID at start of file!\n" );
	}
	...
	if ( formtype != ILBMID && formtype != PBMID ) {
		Com_Error(ERR_DROP, "Unrecognized form type: %c%c%c%c\n", formtype & 0xff
	...
	while ( LBM_P < LBMEND_P )
	{
		...
		case BODYID:
			...
			else
			{
				...
				// unpack ILBM
				//
				Com_Error(ERR_DROP, "%s is an interlaced LBM, not packed", filename );
			}
			break;
		}
...
void LoadPCX( const char *filename, byte **pic, byte **palette, int *width, int *height ){
	...
	if ( len == -1 ) {
		Com_Error(ERR_DROP, "LoadPCX: Couldn't read %s", filename );
	}
	...
	if ( pcx->manufacturer != 0x0a
		 ...
		 || pcx->xmax >= 640
		 || pcx->ymax >= 480 ) {
		Com_Error(ERR_DROP, "Bad pcx file %s", filename );
	}
	...
	if ( !out ) {
		Com_Error(ERR_DROP, "LoadPCX: couldn't allocate" );
	}
	...
	*pic = out;
	...
	/* validity check */
	if ( raw - (byte *) pcx > len ) {
		Com_Error(ERR_DROP, "PCX file %s was malformed", filename );
	}
	free( pcx );
}
...
void LoadBMP( const char *filename, byte **pic, byte **palette, int *width, int *height ){
	...
	if ( len == -1 ) {
		Com_Error(ERR_DROP, "Couldn't read %s", filename );
	}

	i = bufLittleShort( in, len, &pos );
	if ( i != 'B' + ( 'M' << 8 ) ) {
		Com_Error(ERR_DROP, "%s is not a bmp file", filename );
	}
	...
	if ( structSize == 40 ) {
		...
		Com_Error(ERR_DROP, "%s had strange struct size", filename );
	}

	if ( bcPlanes != 1 ) {
		Com_Error(ERR_DROP, "%s was not a single plane image", filename );
	}

	if ( bcBitCount != 8 ) {
		Com_Error(ERR_DROP, "%s was not an 8 bit image", filename );
	}
...
void Load256Image( const char *name, byte **pixels, byte **palette, int *width, int *height ){
	...
	if ( !Q_stricmp( ext, "lbm" ) ) {
		...
		Com_Error(ERR_DROP, "%s doesn't have a known image extension", name );
	}
}

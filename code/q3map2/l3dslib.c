...
void StoreAliasTriangles( void ){
	...
	if ( ( totaltris + numtris ) > MAXTRIANGLES ) {
		Com_Error(ERR_DROP, "Error: Too many triangles" );
	}
...
int ParseVertexL( FILE *input ){
	...
	if ( vertsfound ) {
		Com_Error(ERR_DROP, "Error: Multiple vertex chunks" );
	}
	...
	if ( feof( input ) ) {
		Com_Error(ERR_DROP, "Error: unexpected end of file" );
	}
	...
	if ( numverts > MAXVERTS ) {
		Com_Error(ERR_DROP, "Error: Too many vertices" );
	}
...
int ParseFaceL1( FILE *input ){
	...
	if ( trisfound ) {
		Com_Error(ERR_DROP, "Error: Multiple face chunks" );
	}
...
// OBJ_TRIMESH | {TRI_VERTEXL, TRI_FACEL1}) and skipping other chunks
	...
	if ( vertsfound || trisfound ) {
		Com_Error(ERR_DROP, "Incomplete triangle set" );
	}

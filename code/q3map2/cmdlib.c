...
void *safe_malloc( size_t size ){
	...
	p = malloc( size );
	if ( !p ) {
		Com_Error(ERR_DROP, "safe_malloc failed on allocation of %i bytes", size );
	}
...
void *safe_malloc_info( size_t size, char* info ){
	...
	p = malloc( size );
	if ( !p ) {
		Com_Error(ERR_DROP, "%s: safe_malloc failed on allocation of %i bytes", info, size );
	}
...
int ParseHex( const char *hex ){
	...
	while ( *str )
	{
		...
		if ( *str >= '0' && *str <= '9' ) {
			...
			Com_Error(ERR_DROP, "Bad hex number: %s",hex );
		}
		str++;
	}

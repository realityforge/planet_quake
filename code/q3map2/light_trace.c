		...
		for ( i = 0; i < tw->numVerts; i++ )
		{
			...
			case SIDE_FRONT:
				if ( front->numVerts >= MAX_TW_VERTS ) {
					Com_Error(ERR_DROP, "MAX_TW_VERTS (%d) exceeded", MAX_TW_VERTS );
				}
...
static void FilterTraceWindingIntoNodes_r( traceWinding_t *tw, int nodeNum ){
	...
	if ( node->type >= 0 ) {
		...
		if ( node->children[ 0 ] == 0 || node->children[ 1 ] == 0 ) {
			Com_Error(ERR_DROP, "Invalid tracenode: %d", nodeNum );
		}
...
qboolean TraceTriangle( traceInfo_t *ti, traceTriangle_t *tt, mapTrace_t *trace ){
	int i;
...
qboolean TraceWinding( traceWinding_t *tw, mapTrace_t *trace ){
	int i;
	traceTriangle_t tt;
...
static qboolean TraceLine_r( int nodeNum, vec3_t origin, vec3_t end, mapTrace_t *trace ){
	traceNode_t     *node;
	int side;

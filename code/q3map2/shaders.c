...
shaderInfo_t *CustomShader( shaderInfo_t *si, char *find, char *replace ){
	...
	if ( ( strlen( mapName ) + 1 + 32 ) > MAX_QPATH ) {
		Com_Error(ERR_DROP, "Custom shader name length (%d) exceeded. Shorten your map name.\n", MAX_QPATH );
	}
...
static shaderInfo_t *AllocShaderInfo( void ){
	...
	if ( numShaderInfo == MAX_SHADER_INFO ) {
		Com_Error(ERR_DROP, "MAX_SHADER_INFO exceeded. Remove some PK3 files or shader scripts from shaderlist.txt and try again." );
	}
...
void Parse1DMatrixAppend( char *buffer, int x, vec_t *m ){
	...
	if ( !GetTokenAppend( buffer, qtrue ) || strcmp( token, "(" ) ) {
		Com_Error(ERR_DROP, "Parse1DMatrixAppend(): line %d: ( not found!", scriptline );
	}
	for ( i = 0; i < x; i++ )
	{
		if ( !GetTokenAppend( buffer, qfalse ) ) {
			Com_Error(ERR_DROP, "Parse1DMatrixAppend(): line %d: Number not found!", scriptline );
		}
		m[ i ] = atof( token );
	}
	if ( !GetTokenAppend( buffer, qtrue ) || strcmp( token, ")" ) ) {
...
static void ParseShaderFile( const char *filename ){
	...
	while ( 1 )
	{
		...
		if ( strcmp( token, "{" ) ) {
			if ( si != NULL ) {
				Com_Error(ERR_DROP, "ParseShaderFile(): %s, line %d: { not found!\nFound instead: %s\nLast known shader: %s",
	...
	for ( i = 0; i < count; i++ )
	{
		...
		while ( GetToken( qtrue ) )
		{
			...
			if ( j >= MAX_SHADER_FILES ) {
				Com_Error(ERR_DROP, "MAX_SHADER_FILES (%d) reached, trim your shaderlist.txt!", (int) MAX_SHADER_FILES );
			}

...
void SplitMeshByPlane( mesh_t *in, vec3_t normal, float dist, mesh_t **front, mesh_t **back ){
	...
	if ( f->width > MAX_PATCH_SIZE ) {
		Com_Error(ERR_DROP, "MAX_PATCH_SIZE after split" );
	}
	f->height = in->height;
	...
	for ( i = 4 ; i <= 5 ; i++ ) {
		...
		if ( front ) {
			if ( numOutside == MAX_BRUSH_SIDES ) {
				Com_Error(ERR_DROP, "MAX_BRUSH_SIDES" );
			}
...
void CreateMapFogs( void ){
	...
	for ( i = 0; i < numEntities; i++ )
	{
		...
		for ( brush = entity->brushes; brush != NULL; brush = brush->next )
		{
			...
			if ( numMapFogs >= MAX_MAP_FOGS ) {
				Com_Error(ERR_DROP, "Exceeded MAX_MAP_FOGS (%d)", MAX_MAP_FOGS );
			}
	...
	if ( globalFog[ 0 ] != '\0' ) {
		/* test limit */
		if ( numMapFogs >= MAX_MAP_FOGS ) {
			Com_Error(ERR_DROP, "Exceeded MAX_MAP_FOGS (%d) trying to add global fog", MAX_MAP_FOGS );
		}
		...
		if ( fog->si == NULL ) {
			Com_Error(ERR_DROP, "Invalid shader \"%s\" referenced trying to add global fog", globalFog );
		}
		fog->brush = NULL;
		fog->visibleSide = -1;

...
//		Com_Printf ("=");
		if ( p->leaf == leaf ) {
			Com_Error(ERR_DROP, "CheckStack: leaf recursion" );
		}
		for ( p2 = thread->pstack_head.next ; p2 != p ; p2 = p2->next )
			if ( p2->leaf == p->leaf ) {
				Com_Error(ERR_DROP, "CheckStack: late leaf recursion" );
			}
	}
//	Com_Printf ("\n");
}
...
fixedWinding_t *AllocStackWinding( pstack_t *stack ){
	...
	Com_Error(ERR_DROP, "AllocStackWinding: failed" );
...
void FreeStackWinding( fixedWinding_t *w, pstack_t *stack ){
	...
	for (i = 0; i < sizeof(stack->windings) / sizeof(stack->windings[0]); i++) {
		if (w == &stack->windings[i]) {
			if ( stack->freewindings[i] ) {
				Com_Error(ERR_DROP, "FreeStackWinding: already free" );
			}
	...
	for ( i = 0 ; i < source->numpoints ; i++ )
	{
		...
		for ( j = 0 ; j < pass->numpoints ; j++ )
		{
			...
			if ( ++stack->numseperators[flipclip] >= MAX_SEPERATORS ) {
				Com_Error(ERR_DROP, "MAX_SEPERATORS" );
			}
#endif
	...
	for ( i = 0 ; i < source->numpoints ; i++ )
	{
		...
		for ( j = 0 ; j < pass->numpoints ; j++ )
		{
			...
			if ( numseperators >= maxseperators ) {
				Com_Error(ERR_DROP, "max seperators" );
			}
		...
		for ( j = 0; j < numportals * 2; j++ )
		{
			...
			for ( k = 0; k < numseperators; k++ )
			{
				...
				if ( in.numpoints > MAX_POINTS_ON_FIXED_WINDING ) {
					//% Com_Printf( "[%d]", p->winding->numpoints );
...
//	Com_Printf ("portal %i: %i mightsee\n", portalnum, p->nummightsee);

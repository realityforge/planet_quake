...
qboolean PortalPassable( portal_t *p ){
	...
	if ( p->nodes[ 0 ]->planenum != PLANENUM_LEAF ||
		...
		Com_Error(ERR_DROP, "Portal_EntityFlood: not a leaf" );
	}
...
void AddPortalToNodes( portal_t *p, node_t *front, node_t *back ){
	if ( p->nodes[0] || p->nodes[1] ) {
		Com_Error(ERR_DROP, "AddPortalToNode: allready included" );
	}
...
// remove reference to the current portal
	...
	while ( 1 )
	{
		t = *pp;
		if ( !t ) {
			Com_Error(ERR_DROP, "RemovePortalFromNode: portal not in leaf" );
		}
		...
		if ( t->nodes[0] == l ) {
			...
			Com_Error(ERR_DROP, "RemovePortalFromNode: portal not bounding leaf" );
		}
	}
...
// pad with some space so there will never be null volume leafs
	for ( i = 0 ; i < 3 ; i++ )
	{
		...
		if ( bounds[0][i] >= bounds[1][i] ) {
			Com_Error(ERR_DROP, "Backwards tree volume" );
		}
	}
...
void MakeNodePortal( node_t *node ){
	...
	for ( p = node->portals ; p && w; p = p->next[side] )
	{
		if ( p->nodes[0] == node ) {
			...
			Com_Error(ERR_DROP, "CutNodePortals_r: mislinked portal" );
		}
...
void SplitNodePortals( node_t *node ){
	...
	for ( p = node->portals ; p ; p = next_portal )
	{
		if ( p->nodes[0] == node ) {
			...
			Com_Error(ERR_DROP, "SplitNodePortals: mislinked portal" );
		}

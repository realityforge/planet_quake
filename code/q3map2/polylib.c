...
winding_t   *AllocWinding( int points ){
	...
	if ( points >= MAX_POINTS_ON_WINDING ) {
		Com_Error(ERR_DROP, "AllocWinding failed: MAX_POINTS_ON_WINDING exceeded" );
	}
...
void FreeWinding( winding_t *w ){
	if ( !w ) {
		Com_Error(ERR_DROP, "FreeWinding: winding is NULL" );
	}

	if ( *(unsigned *)w == 0xdeaddead ) {
		Com_Error(ERR_DROP, "FreeWinding: freed a freed winding" );
	}
...
winding_t   *CopyWinding( winding_t *w ){
	...
	if ( !w ) {
		Com_Error(ERR_DROP, "CopyWinding: winding is NULL" );
	}
	...
	for ( i = 0 ; i < w->numpoints ; i++ )
	{
		...
		for ( j = 0 ; j < 3 ; j++ )
			if ( p1[j] > MAX_WORLD_COORD || p1[j] < MIN_WORLD_COORD ) {
				Com_Error(ERR_DROP, "CheckFace: MAX_WORLD_COORD exceeded: %f",p1[j] );
			}

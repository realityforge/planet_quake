...
int CreateNewFloatPlane( vec3_t normal, vec_t dist ){
	...
	if ( nummapplanes + 2 > MAX_MAP_PLANES ) {
		Com_Error(ERR_DROP, "MAX_MAP_PLANES" );
	}
	...
	while ( 1 )
	{
		...
		if ( !strcmp( token, "{" ) ) {
			...
			if ( !strcmp( token, "patchDef2" ) ) {
				...
				if ( g_bBrushPrimit == BPRIMIT_OLDBRUSHES ) {
					Com_Error(ERR_DROP, "Old brush format not allowed in new brush format map" );
				}
			...
			{
				if ( g_bBrushPrimit == BPRIMIT_NEWBRUSHES ) {
					Com_Error(ERR_DROP, "New brush format not allowed in old brush format map" );
				}

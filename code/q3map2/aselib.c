...
void ASE_Load( const char *filename, qboolean verbose, qboolean grabAnims ){
	...
	if ( !fp ) {
		Com_Error(ERR_DROP, "File not found '%s'", filename );
	}
	...
	if ( fread( ase.buffer, ase.len, 1, fp ) != 1 ) {
		fclose( fp );
		Com_Error(ERR_DROP, "fread() != -1 for '%s'", filename );
	}
...
static aseMesh_t *ASE_GetCurrentMesh( void ){
	...
	if ( ase.currentObject >= MAX_ASE_OBJECTS ) {
		Com_Error(ERR_DROP, "Too many GEOMOBJECTs" );
	...
	if ( pObject->anim.currentFrame >= MAX_ASE_ANIMATION_FRAMES ) {
		Com_Error(ERR_DROP, "Too many MESHes" );
		return 0;
	}
...
static void ASE_KeyMATERIAL_LIST( const char *token ){
	if ( !strcmp( token, "*MATERIAL_COUNT" ) ) {
		...
		if ( atoi( s_token ) > MAX_ASE_MATERIALS ) {
			Com_Error(ERR_DROP, "Too many materials!" );
		}
		ase.numMaterials = 0;
	}
...
static void ASE_KeyMESH_VERTEX_LIST( const char *token ){
	...
	if ( !strcmp( token, "*MESH_VERTEX" ) ) {
		...
		if ( pMesh->currentVertex > pMesh->numVertexes ) {
			Com_Error(ERR_DROP, "pMesh->currentVertex >= pMesh->numVertexes" );
		}
	}
	else
	{
		Com_Error(ERR_DROP, "Unknown token '%s' while parsing MESH_VERTEX_LIST", token );
	}
}
...
/*
        ...
        {
            Com_Error(ERR_DROP, "No *MESH_MTLID found for face!" );
        }
 */
	...
	{
		Com_Error(ERR_DROP, "Unknown token '%s' while parsing MESH_FACE_LIST", token );
	}
}
...
static void ASE_KeyTFACE_LIST( const char *token ){
	...
	{
		Com_Error(ERR_DROP, "Unknown token '%s' in MESH_TFACE", token );
	}
}
...
static void ASE_KeyMESH_TVERTLIST( const char *token ){
	...
	if ( !strcmp( token, "*MESH_TVERT" ) ) {
		...
		if ( pMesh->currentVertex > pMesh->numTVertexes ) {
			Com_Error(ERR_DROP, "pMesh->currentVertex > pMesh->numTVertexes" );
		}
	}
	else
	{
		Com_Error(ERR_DROP, "Unknown token '%s' while parsing MESH_TVERTLIST", token );
	}
}
...
static void ASE_KeyMESH( const char *token ){
	...
	if ( !strcmp( token, "*TIMEVALUE" ) ) {
		...
		if ( atoi( s_token ) != pMesh->numFaces ) {
			Com_Error(ERR_DROP, "MESH_NUMTVFACES != MESH_NUMFACES" );
		}
	}
...
static void ASE_KeyMESH_ANIMATION( const char *token ){
	...
	if ( !strcmp( token, "*MESH" ) ) {
		...
		if ( ++ase.objects[ase.currentObject].anim.currentFrame == MAX_ASE_ANIMATION_FRAMES ) {
			Com_Error(ERR_DROP, "Too many animation frames" );
		}
	}
	else
	{
		Com_Error(ERR_DROP, "Unknown token '%s' while parsing MESH_ANIMATION", token );
	}
}
	...
	// loads a sequence of animation frames
		...
		if ( ase.grabAnims ) {
			...
			if ( ase.objects[ase.currentObject].numAnimations ) {
				Com_Error(ERR_DROP, "Multiple MESH_ANIMATIONS within a single GEOM_OBJECT" );
			}
...
static void ASE_Process( void ){
	...
	if ( !ase.currentObject ) {
		Com_Error(ERR_DROP, "No animation data!" );
	}

...
int EmitShader( const char *shader, int *contentFlags, int *surfaceFlags ){
	...
	if ( i == MAX_MAP_SHADERS ) {
		Com_Error(ERR_DROP, "MAX_MAP_SHADERS" );
	}
	numBSPShaders++;
...
void EmitLeaf( node_t *node ){
	...
	if ( numBSPLeafs >= MAX_MAP_LEAFS ) {
		Com_Error(ERR_DROP, "MAX_MAP_LEAFS" );
	}
	...
	for ( dsr = node->drawSurfReferences; dsr; dsr = dsr->nextRef )
	{
		if ( numBSPLeafSurfaces >= MAX_MAP_LEAFFACES ) {
			Com_Error(ERR_DROP, "MAX_MAP_LEAFFACES" );
		}
...
int EmitDrawNode_r( node_t *node ){
	...
	if ( numBSPNodes == MAX_MAP_NODES ) {
		Com_Error(ERR_DROP, "MAX_MAP_NODES" );
	}
	...
	if ( node->planenum & 1 ) {
		Com_Error(ERR_DROP, "WriteDrawNodes_r: odd planenum" );
	}
...
void SetLightStyles( void ){
	...
	for ( i = 1; i < numEntities; i++ )
	{
		...
		if ( style < LS_NORMAL || style > LS_NONE ) {
			Com_Error(ERR_DROP, "Invalid lightstyle (%d) on entity %d", style, i );
		}
		...
		if ( j >= numStyles ) {
			if ( numStyles == MAX_SWITCHED_LIGHTS ) {
				Com_Error(ERR_DROP, "MAX_SWITCHED_LIGHTS (%d) exceeded, reduce the number of lights with targetnames", MAX_SWITCHED_LIGHTS );
			}
...
void EmitBrushes( brush_t *brushes, int *firstBrush, int *numBrushes ){
	...
	for ( b = brushes; b != NULL; b = b->next )
	{
		...
		if ( numBSPBrushes == MAX_MAP_BRUSHES ) {
			Com_Error(ERR_DROP, "MAX_MAP_BRUSHES (%d)", numBSPBrushes );
		}
		...
		for ( j = 0; j < b->numsides; j++ )
		{
			...
			if ( numBSPBrushSides == MAX_MAP_BRUSHSIDES ) {
				Com_Error(ERR_DROP, "MAX_MAP_BRUSHSIDES " );
			}
...
void BeginModel( void ){
	...
	if ( numBSPModels == MAX_MAP_MODELS ) {
		Com_Error(ERR_DROP, "MAX_MAP_MODELS" );
	}

  ...
  int num_indices, qhandle_t texture, const vec2_t translation)
{
  ...
  if(texture)
  {
      ...
      qglBindTexture(texture, &texw, &texh);
  }

  for(int  i = 0; i < num_vertices; i++) {
    memcpy(&verts[i], &sourceVerts[(i*4)*5+0], sizeof(vec2_t));
    memcpy(&colors[i], &sourceVerts[(i*4)*5+2], sizeof(int));
    memcpy(&coords[i], &sourceVerts[(i*4)*5+3], sizeof(vec2_t));
    if (texture) {
      memcpy(&coords[i], &sourceVerts[(i*4)*5+3], sizeof(vec2_t));
      coords[i] *= tr.shaders[texture]->bundle[0].image[0]->width
      coords[i] *= tr.shaders[texture]->bundle[0].image[0]->height

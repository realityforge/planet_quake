...
#define MAX_ALIAS (ALIAS * ALIAS * ALIAS)
	...
	byte *imageData = Z_Malloc( IMAGE_SIZE * IMAGE_SIZE * 4 + 18 );
	memset( imageData, 0, 18 );

	static byte alphaNumeric[95][13] = {
		...
		/* possibly backwards */ {0x00, 0x60, 0x60, 0x30, 0x30, 0x18, 0x18, 0x0c, 0x0c, 0x06, 0x06, 0x03, 0x03},
...
#define CHAR_RANGE 128
			...
			if(coordX <= RASTER_BORDER_X || coordY <= RASTER_BORDER_Y ) {
				...
				for(int j = -(ALIAS/2); j < ALIAS/2; j++) {
					for(int k = -(ALIAS/2); k <  ALIAS/2; k++) {
						...
						if(j == 0 || k == 0) continue;
						qboolean bitSet = bitCharacter[bitY + k] & (1 << (bitX + j));
						if(!bitSet) {
							bitCount--;
						}
					}
	...
	imageData[2] = 2;      // uncompressed type
	imageData[12] = IMAGE_SIZE & 255;
	imageData[13] = IMAGE_SIZE >> 8;
	imageData[14] = IMAGE_SIZE & 255;

...
static void SetupLinear( textureLevel_t **pic, int width, int height, int depth,
	...
	*pic = lvl = ri.Malloc( mipmaps * sizeof( textureLevel_t ) + size * depth );
	...
	for( i = 0; i < mipmaps; i++ ) {
		...
		lvl[i].width = w;
		lvl[i].height = h;
		lvl[i].size = size * sizeof(byte[4]) * depth;
		lvl[i].data = (byte *)out;
	...
	for( k = 0; k < depth; k++ ) {
		for( i = 0; i < mipmaps; i++ ) {
			...
			out = (byte *)lvl[i].data + k * size * sizeof(byte[4]);
			in = (byte *)base + k * size * bitCount / 8;
...
void R_LoadDDS( const char *name, byte **outputPic, int *width, int *height, GLenum *picFormat, int *numTexLevels )
{
	union {
		byte *b;
		void *v;
	...
	byte  *base;
	int    depth;
  textureLevel_t **pic = NULL;

  int mipsRequested = *numTexLevels;
	*outputPic = NULL;
	*numTexLevels = 0;
...
// should we support compressed luminance-alpha formats too ?
  ...
  outputPic = pic[mipsRequested]->data;
  *height = pic[mipsRequested]->height;
  *width = pic[mipsRequested]->width;
  *numTexLevels = pic[mipsRequested]->format;

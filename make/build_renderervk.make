...
REND_TARGET    := $(CNAME)_vulkan_$(SHLIBNAME)
ifeq ($(USE_MULTIVM_CLIENT),1)
REND_TARGET    := $(CNAME)_mw_vulkan_$(SHLIBNAME)
endif

WORKDIRS += $(REND_WORKDIR) $(REND_WORKDIR)/shaders
CLEANS 	 += $(REND_WORKDIR) $(REND_TARGET)

REND_SOURCES   := $(MOUNT_DIR)/$(REND_SOURCE) \
									$(MOUNT_DIR)/renderercommon
#									$(MOUNT_DIR)/$(REND_SOURCE)/shaders 
#GLSLFFALLBACKS := $(foreach dir,$(REND_SOURCES), $(wildcard $(dir)/*.glsl))
#GLSLFILES      := $(addprefix shaders/,$(notdir $(GLSLFFALLBACKS)))
# TODO: switch this when other map formats are working
REND_CFILES    := $(foreach dir,$(REND_SOURCES), $(filter-out $(dir)/tr_font.c,$(wildcard $(dir)/*.c))) \
									$(MOUNT_DIR)/$(REND_SOURCE)/tr_font.c
...
#                  $(addprefix $(B)/$(REND_WORKDIR)/shaders/,$(notdir $(Q3R2STRINGOBJ)))
Q3R2STRCLEAN   := $(addsuffix _clean,$(addprefix $(B)/$(REND_WORKDIR)/shaders/,$(notdir $(Q3R2STRINGOBJ))))
...
$(B)/$(REND_WORKDIR)/shaders/%.c: $(MOUNT_DIR)/$(REND_SOURCE)/shaders/%.glsl
	$(DO_REF_STR)

$(B)/$(REND_WORKDIR)/shaders/%.o_clean: $(MOUNT_DIR)/$(REND_SOURCE)/shaders/%.glsl
	@rm -f ./$@

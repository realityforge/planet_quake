...
Q3MAP2_OBJ       := $(addprefix $(B)/$(WORKDIR)/,$(notdir $(Q3MAP2))) \
								    $(addprefix $(B)/$(WORKDIR)/picomodel/,$(notdir $(PICOMODEL)))
...
define DO_Q3MAP2_GXX
  $(echo_cmd) "Q3MAP2_GXX $<"
  $(Q)$(GXX) $(SHLIBCFLAGS) $(CXXFLAGS) -o $@ -c $<
endef
...
debug:
	...
	@$(MAKE) -f $(MKFILE) B=$(BD) V=$(V) WORKDIRS="$(WORKDIR) $(WORKDIR)/picomodel" mkdirs
...
release:
	$(echo_cmd) "MAKE $(Q3MAP2_TARGET)"
	@$(MAKE) -f $(MKFILE) B=$(BR) V=$(V) WORKDIRS="$(WORKDIR) $(WORKDIR)/picomodel" mkdirs
...
$(B)/$(WORKDIR)/picomodel/%.o: $(Q3MAP2DIR)/../libs/picomodel/%.c
	$(DO_Q3MAP2_CC)
...
$(B)/$(Q3MAP2_TARGET): $(Q3MAP2_OBJ) 
	$(echo_cmd) "LD $@"
	$(Q)$(CC) -o $@ $(Q3MAP2_OBJ) $(LIBS) $(Q3MAP2_LDFLAGS)
endif

...
#REPLACE_EXTS      = $(foreach vext,$(addprefix %.,$(1)), $(foreach iext,$(addprefix %.,$(2)), $(patsubst $(iext),$(vext),$(3))))
COPY             := cp
UNLINK           := rm
MOVE             := mv
MKDIR            ?= mkdir -p
...
RPK_REPLACE       = $(SRCDIR)/$(subst $(DESTDIR)/$(RPK_TARGET)dir/,,$(subst \space, ,$(1)))
RPK_GETEXT        = $(basename $(subst \space, ,$(1))).$(call GETEXT,$(call RPK_REPLACE,$(1)))
...
RPK_CONVERT      += $(subst .pk3,-converted.pk3,$(RPK_TARGET))
...
define DO_CONVERT_CC
	...
	$(Q)$(CONVERT) "$(call RPK_REPLACE,$@)" "$(call RPK_GETEXT,$@)"
endef
...
#IMAGE_ALTEXTS    := $(call REPLACE_EXTS,$(IMAGE_VALID_EXTS),$(IMAGE_CONV_EXTS),$(IMAGE_NEEDED))
...
$(DESTDIR)/$(PK3_PREFIX)-converted.pk3: $(IMAGE_OBJS)
	$(echo_cmd) "CONVERTED $@"
	@:
